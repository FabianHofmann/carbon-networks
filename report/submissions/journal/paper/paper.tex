...
The European Union's goal to achieve climate neutrality by 2050 involves the implementation of carbon management strategies as a crucial component. This objective is integrated into a wide range of programs, funding schemes and initiatives~\cite{eu2023netzero,europeangreendeal,europeaninnovationfund}.

%~\cite{lockwoodEuropeanStrategyCarbon} - Clean Air Task Force
%~\cite{righettiSitingCarbonDioxide2017} - co2 pipeline are a mature technology, references to US and Canada co2 pipelines used for enhanced oil recovery
%~\cite{friedmannNETZEROGEOSPHERICRETURN} - high learning rate for co2 pipeline costs
%~\cite{NorthernLightsWhat} - Northern Lights project in Norway, 1.5 Mt/a co2 transport capacity, 1.5 Mt/a co2 storage capacity, 2024 start of operation, targeted scale of 5 Mtpa CO2 Seq. by 2030.
...
% point out that~\cite{morbeeOptimisedDeploymentEuropean2012} optimizes the topology and capacity of a co2 network, but only considers the power sector without co-optimizing renewable deployment.
...
Taking non-abatable emissions from fossil-based industrial processes, aviation, shipping and agriculture into account, the system emits of 633~Mt \carbon{} per year, 153~Mt of which are industrial emissions which can optionally be captured through corresponding investments.


Carbon-neutral energy is provided by wind, solar, biomass and hydro. Weather-dependent power potentials for solar, wind and hydro per region and time-stamp are calculated from reanalysis and satellite data sets Copernicus ERA5~\cite{hersbachERA5GlobalReanalysis2020} and Surface Radiation Data Set - Heliosat~\cite{pfeifrothSurfaceRadiationData2017}  using the open-source tool~\cite{hofmannAtliteLightweightPython2021}.
Solar and wind power can be expanded in alignment with eligible land-use restrictions calculated on the basis of~\cite{eeaCorineLandCover2012,eeaNatura2000Data2016}. The biomass potential is calculated from the European Commission's Joint Research Centre's ENSPRESO database~\cite{enspreso_database} and the JRC-EU-TIMES model~\cite{instituteforenergyandtransportjointresearchcentreJRCEUTIMESModelBioenergy2015}. Its usage in each region is bound by its biomass potential, but inter-regional biomass transport is permitted with transport costs considered.


Salt caverns for \hydrogen{} storage are derived from~\cite{caglayanTechnicalPotentialSalt2020}.
...
% hight level trends
- cost reduction with carbon network is significant (2.5\%)
...
\begin{figure}
    ...
    \caption{Share of facilities with integrated carbon capture as a function of emission targets for system with \hydrogen{} and \carbon{} network.}
